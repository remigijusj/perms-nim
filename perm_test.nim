...
suite "constructors":
  test "direct":
    let p: Perm[N] = [1'u8, 0'u8, 2'u8, 3'u8, 4'u8, 5'u8, 6'u8, 7'u8]
    check(p == [1, 0, 2, 3, 4, 5, 6, 7])

  test "newPerm invalid":
    var p: Perm[N]
    expect PermError:
      p = newPerm(@[1, 2, 3, 4, 5, 6, 7], N)
    expect PermError:
      p = newPerm(@[0, 0, 1], N)
    expect PermError:
      p = newPerm(@[-2, 0, 1], N)
  ...
  test "toPerm 0":
    let c = newCycle(@[1, 2, 3], N)
    check(c.toPerm(N) == [0, 2, 3, 1, 4, 5, 6, 7])

  test "toPerm 1":
    let c = newCycle(@[3], N)
    check(c.toPerm(N) == identity(N))
...
suite "basics":
  ...
  test "isInvolution 2":
    let p = newPerm(@[1, 2, 0], N)
    check(p.isInvolution == false)

  test "isInvolution 3":
    let p = newPerm(@[1, 0, 3, 2], N)
    check(p.isInvolution == true)

  test "isInvolution 3":
    let p = newPerm(@[1, 2, 3, 0], N)
    check(p.isInvolution == false)
  ...
  test "isEqual 2":
    let p = newPerm(@[0, 1], N)
    let q = newPerm(@[0, 1, 2], N)
    check(p == q)
    check(q == p)

  test "isEqual c0":
    let c = newCycle(@[1, 2, 3], N)
    let d = newCycle(@[2, 3, 1], N)
    check(c == d)

  test "isEqual c1":
    let c = newCycle(@[3, 1, 2], N)
    check(c == @[1, 2, 3])


suite "actions":
  test "inverse":
    let p = newPerm(@[1, 2, 3, 4, 0], N)
    check(p.inverse == [4, 0, 1, 2, 3, 5, 6, 7])


  test "compose 0":
    let p = newPerm(@[1, 2, 0], N)
    let q = newPerm(@[0, 3, 4, 1, 2], N)
  ...
  test "conjugate 2":
    let p = newPerm(@[4, 2, 0, 1, 3], N)
    let q = newPerm(@[1, 2, 0], N)
    check(p.conjugate(q) == [1, 4, 0, 2, 3, 5, 6, 7])

  test "conjugate c0":
    let c = newCycle(@[0, 1, 2, 3], N)
    let q = newPerm(@[4, 3, 2, 1, 0], N)
    check(c.conjugate(q) == @[1, 4, 3, 2])

  test "conjugate c1":
    let c = newCycle(@[0, 1, 2], N)
    let q = newPerm(@[2, 0, 1, 4, 5, 3], N)
    check(c.conjugate(q) == c)

  test "conjugate c2":
    let c = newCycle(@[0, 1, 2], N)
    let q = newPerm(@[0, 1, 2, 4, 3], N)
    check(c.conjugate(q) == c)

  test "conjugate c2":
    let c = newCycle(@[0, 4, 7], N)
    let q = randomPerm(N)
    check(c.conjugate(q).toPerm(N) == c.toPerm(N).conjugate(q))
...
suite "signature":
  ...
  test "signature 5a":
    let p = newPerm(@[1, 0, 3, 2, 4], N)
    check(p.signature == [0, 4, 2, 0, 0, 0, 0, 0, 0])
    check(p.sign == 1)
    check(p.order == 2)

  test "signature 5c":
    let p = newPerm(@[1, 0, 3, 4, 2], N)
  ...
  test "signature 6b":
    let p = newPerm(@[0, 2, 1, 4, 5, 3], N)
    check(p.signature == [0, 3, 1, 1, 0, 0, 0, 0, 0])
    check(p.sign == -1)
    check(p.order == 6)

  test "signature 6c":
    let p = newPerm(@[5, 4, 1, 2, 3, 0], N)
...
suite "cycles":
  test "parsePerm invalid 0":
    expect PermError:
      discard parsePerm("(1 2 3 0)", N)

  test "parsePerm invalid 1":
    expect PermError:
      discard parsePerm("(1 2)(2, 3)", N)

  test "parsePerm invalid 2":
    expect PermError:
      discard parsePerm("(1, 2, 65537)", N)
  ...
  test "parsePerm 4":
    let p = parsePerm("(1,2)", N)
    check(p == [1, 0, 2, 3, 4, 5, 6, 7])

  test "parsePerm 5":
    let p = parsePerm("(3,5)", N)
    check(p == [0, 1, 4, 3, 2, 5, 6, 7])

  test "parsePerm 6":
    let p = parsePerm("(1, 2) (3, 4) ", N)
    check(p == [1, 0, 3, 2, 4, 5, 6, 7])

  test "parsePerm 7":
    let p = parsePerm("(1 2)(3 8)(7 4)", N)
  ...
  test "parsePerm 9":
    let p = parsePerm("1 2 3 4)(5 6 7 8", N)
  ...
  test "printCycles 5":
    let p = newPerm(@[1, 2, 0, 4, 5, 3], N)
    check(p.cycles[0] == @[0, 1, 2])
    check(p.cycles[1] == @[3, 4, 5])
    check(p.printCycles == "(1, 2, 3)(4, 5, 6)")

  test "printCycles 6":
    let p = newPerm(@[5, 4, 3, 2, 1, 0], N)
    check(p.printCycles == "(1, 6)(2, 5)(3, 4)")

  test "printCycles 7":
    let p = newPerm(@[0, 1, 4, 3, 2, 5], N)
  ...
  test "splitCycles3 1":
    let p = parsePerm("(1 2 4 8)(3 5)", N)
    let s = @[@[0, 1, 3], @[0, 7, 2], @[0, 4, 2]]
    for i, c in p.splitCycles(3):
      check(c == s[i])

  test "splitCycles3 2":
    let p = parsePerm("(1 7)(3 2 6)(5 8)", N) # [0 6][1 5 2][4 7]
