...
# Parse base with names. Recognizes the format produced by printBase.
...
# Print base elements in cycle notation, with names. One per line.
...
# Generate random base of desired size. Possibly enforce distinct elements.
...
# Find base element by name. Return None if not found.
...
# Return sequence of plain perms of the base.
proc perms*[N: static[int]](base: PermBase[N]): seq[Perm[N]] =
  result = base.mapIt(it.perm)


# Calc the sign of the base: -1 if any element is odd, 1 otherwise.
...
# "Normalize" the base by adding inverses (except for involutions).
...
# Return true if the group generated by the base is transitive (has single orbit)
...
# Compose a sequence of base elements given by indices
...
# Produce human-readable format of a factorization from a sequence of base indices.
# Example:
...
# Decode an index from the given level, get a sequence of base indices
...
# Multiply the given list of perms with each of the base elements.
# Produce both a perm and index useful for reconstructing it back from the base.
...
# Perform Breadth-First Search by composing base elements up to given # of levels.
...
# Produce 2 lists:
# c: cycles found
# s: sequences of base indices leading to the corresponding cycle.
#    Residual order is pushed as the last element of the list.
proc searchCycle*[N: static[int]](base: PermBase[N]; length, levels: int; max = 0; full = false): tuple[c: seq[Cycle[N]], s: seq[seq[int]]] =
  result.c = @[]
  result.s = @[]
  for p, i, level in base.multiSearch(levels):
    ...
    if o > -1:
      ...
      if not result.c.contains(c):
        var s = decodeIndex(i, level, base.len)
...
# Conjugate the given list of perms with each of the base elements.
# Produce a perm, index of original perm  in the list, index of conjugating perm in the base.
# Similar to `multiply`.
...
# Perform Breadth-First Search by conjugating seed (list of cycles) with base elements.
...
# Factorize given target perm to a sequence of base elements using Kalka algorithm.
# Return sequence of base indices. Use factorNames to obtain a nicer format.
# Optional params:
